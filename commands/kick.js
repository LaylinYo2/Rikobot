...
module.exports.run = async (bot, message, args) => {
    ...
    let kickChannel = message.guild.channels.find(c => c.name === "modlog");
    if(!kickChannel) return message.channel.send("Couldn't find log channel.");

    kickUser.send(`You've been **kicked** from **${message.guild.name}** for: **${kickReason}**. Use this invite to join back -> https://discord.gg/knobbelboy `).then(function() {
    ...
    let memberKickLog = message.guild.channels.find(c => c.name === "stafflogs");
