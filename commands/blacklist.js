...
module.exports.run = async (bot, message, args) => {
    ...
    if(!message.member.roles.has("370347770864861194")) return message.channel.send("Insufficient permission.");
    ...
    if(blacklistUser.roles.has("368170751163105280")) return message.channel.send(":clown: You tried. :clown:");
    ...
    let blacklistChannel = message.guild.channels.find(c => c.name === "modlog")
    ...
    let blacklists = message.guild.channels.find(c => c.name === "wall_of_shame");
    ...
    let memberBlacklistLog = message.guild.channels.find(c => c.name === "stafflogs");
