...
bot.on("messageUpdate", (message, oldMessage, newMessage) => {
  ...
  let eliteChannel = bot.channels.find(r => r.name === 'elite-room');
  if(message.channel.name === 'elite-room') return;
  ...
  if (textInput.length < "1248") {
    ...
    const messagechannel = message.guild.channels.find(r => r.name === 'stafflogs');
